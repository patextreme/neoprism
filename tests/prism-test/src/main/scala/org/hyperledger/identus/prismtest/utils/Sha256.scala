...
private[utils] case class Sha256HashImpl(bytes: Vector[Byte]) extends Sha256Hash:
...
object Sha256Hash:
  ...
  def fromHex(hexedBytes: String): Sha256Hash =
    ...
    if HEX_STRING_RE.matches(hexedBytes) then Sha256HashImpl(hexToBytes(hexedBytes))
    else
