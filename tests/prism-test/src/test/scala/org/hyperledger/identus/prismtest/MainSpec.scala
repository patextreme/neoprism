...
object MainSpec extends ZIOSpecDefault, TestUtils:
  ...
  override def spec =
    val prismNodeSpec = suite("PRISM node suite")(createOperationSuite)
      .provide(NodeClient.grpc("localhost", 50053), NodeName.layer("prism-node"))

    val scalaDidSpec = suite("scala-did node suite")(createOperationSuite)
      .provide(NodeClient.grpc("localhost", 8980), NodeName.layer("scala-did"))

    val neoprismSpec = suite("NeoPRISM suite")(createOperationSuite)
      .provide(
